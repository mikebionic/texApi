...
func GetDestailedCargoList(ctx *gin.Context) {
	page, _ := strconv.Atoi(ctx.DefaultQuery("page", "1"))
	perPage, _ := strconv.Atoi(ctx.DefaultQuery("per_page", "10"))
	companyID, _ := strconv.Atoi(ctx.DefaultQuery("companyID", "10"))
	offset := (page - 1) * perPage

	stmt := queries.GetDetailedCargoList
	...
	ctx.JSON(http.StatusOK, utils.FormatResponse("Detailed cargo list", response))
}
