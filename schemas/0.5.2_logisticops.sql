...
-- Мои отклики
CREATE TYPE bid_state_t AS ENUM ('pending', 'accepted', 'declined');
...
(
    ...
    state      bid_state_t NOT NULL                                    DEFAULT 'pending',
    created_at TIMESTAMP                                                    DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP                                                    DEFAULT CURRENT_TIMESTAMP,
    deleted    INT
);
...
INSERT INTO tbl_request (user_id, company_id, driver_id, vehicle_id, cost_per_km, from_country, from_region, to_country, to_region, view_count, validity_start, validity_end, note)
...
INSERT INTO tbl_bid (company_id, bid_user_id, state)
VALUES    (2, 2, 'declined'),
    (3, 3, 'accepted'),
    (1, 1, 'pending'),
    (2, 2, 'declined'),
